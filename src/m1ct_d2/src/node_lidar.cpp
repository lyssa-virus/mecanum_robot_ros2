...
int send_lidar_data(LaserScan &outscan)
{
	...
	{	
		...
		outscan.config.min_angle = 0.0;
		outscan.config.max_angle = 2.0 * M_PI - outscan.config.angle_increment;
		...
		if(node_lidar.lidar_status.isConnected)
		{
			...
			for (int i = static_cast<int>(count) - 1; i >= 0; i--)
			{
				...
				int angle_index = static_cast<int>(count) - 1 - i;

				angle = static_cast<float>((node_lidar.scan_node_buf[angle_index].angle_q6_checkbit >>
...
if(data_handling(scan))
{
    ...
    // O slam_toolbox exige que a geometria do LaserScan
    // permaneça constante depois que o sensor é registrado.
    constexpr size_t NUM_LASER_POINTS = 400;

    scan_msg->ranges.resize(NUM_LASER_POINTS);
    scan_msg->intensities.resize(NUM_LASER_POINTS);

    // O timestamp representa aproximadamente o início da varredura.
    scan_msg->header.stamp =
        node->now() -
        rclcpp::Duration::from_seconds(scan.config.scan_time + 0.02);

    scan_msg->header.frame_id =
        node_lidar.lidar_general_info.frame_id;

    // Uma volta completa dividida exatamente em 400 amostras.
    scan_msg->angle_min = 0.0;

    scan_msg->angle_increment =
        (2.0 * M_PI) /
        static_cast<double>(NUM_LASER_POINTS);
	
    scan_msg->angle_max =
        scan_msg->angle_min +
        static_cast<double>(NUM_LASER_POINTS - 1) *
        scan_msg->angle_increment;
    ...
    // Copia no máximo 400 pontos.
    const size_t points_to_copy =
        std::min(scan.points.size(), NUM_LASER_POINTS);

    for(size_t i = 0; i < points_to_copy; i++)
    {
        const float range =
            static_cast<float>(scan.points[i].range);

        // Valores menores que range_min, maiores que range_max
        // ou iguais a zero são tratados como ausência de retorno.
        ...
        scan_msg->intensities[i] =
            static_cast<float>(scan.points[i].intensity);
    }

    // Se a volta vier com 399 pontos, por exemplo,
    // completa o restante para manter exatamente 400.
    for(size_t i = points_to_copy;
        i < NUM_LASER_POINTS;
        i++)
    {
        scan_msg->ranges[i] =
